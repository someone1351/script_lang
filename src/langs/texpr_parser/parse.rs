...
fn parse_block_begin(input:&mut Input) -> Option<BlockBracket> {
    let x=input.has(0, ["{","[","("]);
...
fn parse_block_end(input:&mut Input) -> Option<BlockBracket> {
    let x=input.has(0, ["}","]",")"]);
...
fn parse_cmnt(input:&mut Input) -> bool {
    if Some("#")!=input.get(0, 1) {
        return false;
    }
...
fn parse_end(input:&mut Input) -> bool {
    if let Some(x)=input.has(0, ["\r\n","\n"]) { //";",
        input.next(x.len());
        true
    } else {
...
fn parse_semi_colon(input:&mut Input) -> bool {
    if let Some(x)=input.has(0, [";"]) {
        input.next(x.len());
        true
    } else {
