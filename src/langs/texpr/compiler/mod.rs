...
use crate::{primitive_types::StringVal, texpr::parser};
...
use super::parser::*;
use crate::compiler::{ast, builder,  };
use cmds::*;
pub use error::*;
...
impl Compiler {
    ...
    fn get_cmds(&self,k:&str) -> Option<std::slice::Iter<'_, Cmd>> {
                    ...
                    if let Some(first_param)=record.first_param() {
                        ...
                        if let Some(symbol)=first_param.as_primitive().as_symbol() {
                            if ["true","false","nil","void"].contains(&symbol) || symbol.starts_with(":") {
                                ...
                                if let Some(cmds)=self.get_cmds(symbol) {  //command
